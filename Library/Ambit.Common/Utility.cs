...
namespace Ambit.Common
{
    ...
    public static class Utility
    {
        ...
        /// <param name="AmbitqueCon">The Ambitque con.</param>
        /// <param name="startWith">The start with.</param>
        /// <returns>
        /// The data table
        /// </returns>
        public static DataTable CreateDataTable<T>(IEnumerable<T> list, List<string> AmbitqueCon = null, string startWith = "")
        {
            try
            {
                Type type = typeof(T);
                ...
                if (AmbitqueCon != null)
                {
                    DataColumn[] arrayDC = new DataColumn[AmbitqueCon.Count];
                    for (int i = 0; i < AmbitqueCon.Count; i++)
                    {
                        arrayDC[i] = dataTable.Columns[AmbitqueCon[i]];
                    }

                   UniqueConstraint custAmbitque = new UniqueConstraint(arrayDC);
                    dataTable.Constraints.Add(custAmbitque);
                }

...
namespace Ambit.Common
{
    ...
    public class CommonHelper
    {
        ...
        /// <summary>
        /// Generate15s the Ambitque digits.
        /// </summary>
        /// <returns></returns>
        public static long Generate15AmbitqueDigits()
        {
            lock (_locker)
            {
                Thread.Sleep(100);
        ...
        public static void Logtxt(string directory, string messagetxt)
        {
            string message = string.Format("Time: {0}", DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt"));
            message += Environment.NewLine;
            message += "-----------------------------------------------------------";
            message += Environment.NewLine;
            message += string.Format("Message: {0}", messagetxt);
            message += Environment.NewLine;
            message += "-----------------------------------------------------------";
            message += Environment.NewLine;
            string path = directory;//Server.MapPath("~/ErrorLog/ErrorLog.txt");
    ...
    {
        public static string SendOtp(string mobileNumber, string message = "", string ApplicationCode = "")
        {
            try
            {
                //if (!Configurations.TestMode)
                //{
                //CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", mobileNumber);
                //CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", message);
                ...
                //// Twilio Code
                //// My watchman twilio
                string accountSid = Configurations.accountSid;
                string authToken = Configurations.authToken;

                TwilioClient.Init(accountSid, authToken);

                var messageTwilio = MessageResource.Create(
                body: message + " " + ApplicationCode,
                to: new Twilio.Types.PhoneNumber(mobileNumber),
                from: new Twilio.Types.PhoneNumber(Configurations.TwilioFrom)
                );

                //string from = Configurations.SenderId;
                //string to = mobileNumber;
                //string text = message;
                //CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", from);
                //CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", to);
                //CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", text);
                //CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", count.ToString());

                //Thread.Sleep(1000);

                //if (client.SendSms(from, to, text))
                //    CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", "Done");
                //else
                //    CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", "Try Again");

                //    count = count + 1;
                //} while (count < 5);

                //client.Disconnect();
                //CommonHelper.Logtxt(@"C:\inetpub\wwwroot\api\Log\ErrorLog.txt", "Disconnect");
                return "true";
                //}
                //else
                //{
        ...
        public static bool Send(string mobileNumber, string otp = "", string message = "")
        {
            bool IsSuccess = false;
            HttpClient client = new HttpClient();
            if (Configurations.OtpPopup == 1)
            {
                ...
                message =  "OTP%20for%20Registration%20on%20QUPA%20Exam%20Generator%20is " +otp ;
